...
if (!empty($_REQUEST['lotnumber'])) {
	...
	$sql = "SELECT * FROM ng2_retest WHERE lot_no LIKE '%".$lotnumber."%' ORDER BY id desc";
	...
	if($count>0){
		...
		echo '<script type="text/javascript">function hidden(){document.getElementById("ngretest2").style.display = "none";}</script>';
		echo '<script type="text/javascript">hidden()</script>';
	}
}else{
	$sql = "SELECT * FROM ng2_retest ORDER BY id desc";
	$r_query = mysqli_query($conn,$sql);
